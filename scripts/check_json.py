...
# 取得檔案路徑
file_path = Path('/data/AI_tools/RAG_v2/output/gear_full_output.json')
        ...
        if data and len(data) > 0:
            ...
            if 'metadata' in first_item:
                ...
                if 'embedding' in metadata:
                    ...
                    print(f"embedding 長度: {len(embedding) if isinstance(embedding, list) else 'not a list'}")
            ...
            # 檢查是否有表格和圖片
            has_table = 'table_html' in first_item.get('metadata', {}) or first_item.get('filetype') == 'table/html'
            has_image = 'image_base64' in first_item.get('metadata', {}) or first_item.get('filetype') == 'image'
